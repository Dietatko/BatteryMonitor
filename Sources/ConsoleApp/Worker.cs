...
namespace ImpruvIt.BatteryMonitor.ConsoleApp
{
    public class Worker : IHostedService
    {
        ...
        private void DoWork(CancellationToken stoppingToken)
        {
            ...
            MonitorBattery().GetAwaiter().GetResult();
        }
        
        private async Task MonitorBattery()
		{
			// Discover bus devices
			IEnumerable<IDiscoverDevices> discoveryServices = GetDiscoveryServices();
			var discoveryTask = Task.WhenAll(discoveryServices.Select(x => x.GetConnectors()));
			...
			try
			{
                ...
                var batteryAdapter = new BatteryAdapter(
	                new SMBusInterface((ICommunicateToAddressableBus)connection), 
	                address, 
	                serviceProvider.GetRequiredService<ILogger<BatteryAdapter>>());
		...
		private void TestLtc6804()
		{
			...
			var adapter = new LTC6804.BatteryAdapter(connection, serviceProvider.GetRequiredService<ILogger<LTC6804.BatteryAdapter>>());
        ...
        public Task StartAsync(CancellationToken stoppingToken)
        {
            ...
            workerTask = Task.Run(() => DoWork(stoppingToken), stoppingToken);
        ...
        public Task StopAsync(CancellationToken stoppingToken)
        {
            ...
            return Task.WhenAll(workerTask);
        }
    }
}